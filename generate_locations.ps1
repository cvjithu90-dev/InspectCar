...
$out_dir = "C:\Users\Jithu\.gemini\antigravity\scratch\inspectcar-booking\locations"
...
$all_slugs = $locations | ForEach-Object { $_[1] }
$cards = ""
...
foreach ($loc in $locations) {
    ...
    $hero_desc = "Looking to buy a used car in ${loc_name}? Our expert mechanics provide thorough doorstep pre-purchase vehicle inspections in this $area_desc. Don't leave your car purchase to chance."
    $content_p1 = "Buying a second-hand vehicle in a $area_desc like $loc_name comes with its own set of challenges. Whether you're checking out cars parked in apartment complexes or meeting sellers near the major tech hubs, our inspectors travel directly to your location in $loc_name and surrounding localities. We handle all the technical checks so you can make a confident buying decision."
    $content_p2 = "We frequently inspect a variety of vehicles in this area, especially $car_types which are highly sought after by local residents. From hidden accident damage to engine health and odometer tampering, our comprehensive 200+ point checklist covers everything. We've helped hundreds of buyers in $loc_name avoid expensive repair bills."
...
"@
    ...
    $other_slugs = $all_slugs | Where-Object { $_ -ne $slug } | Get-Random -Count 8
    $loc_tags = ""
    foreach ($s in $other_slugs) {
        $s_name = ($locations | Where-Object { $_[1] -eq $s })[0]
        $loc_tags += "<a href=`"../locations/$s.html`" class=`"loc-tag`">$s_name</a>`n    "
    }
    
    $html = $template -replace '\{location\}', $loc_name -replace '\{slug\}', $slug -replace '\{hero_desc\}', $hero_desc -replace '\{content_p1\}', $content_p1 -replace '\{content_p2\}', $content_p2 -replace '\{faqs\}', $faqs -replace '\{loc_tags\}', $loc_tags
    
    $out_file = Join-Path $out_dir "$slug.html"
    Set-Content -Path $out_file -Value $html -Encoding UTF8
...
$index_html = $index_template -replace '\{cards\}', $cards
$index_out_file = Join-Path $out_dir "index.html"
Set-Content -Path $index_out_file -Value $index_html -Encoding UTF8
